...
def check_title_page(pdf_path):
    ...
    try:
        ...
        for page_num, page in enumerate(reader.pages):  # Check entire document
...
def check_pdf_readability(pdf_path, threshold=50):
    ...
    try:
        ...
        ocr_confidences = []
        for image in images:  # Process all pages
            try:
...
st.write(
    ...
    "The app will analyze the entire document for text extraction and OCR analysis."
)

# Use columns to create a responsive layout
col1, col2 = st.columns([2, 1])  # Two columns: the first takes 2/3 of the space, the second 1/3
